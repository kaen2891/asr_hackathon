...
import label_loader
from loader import *
from models.transformer import Model #3d use
...
def bind_model(model, optimizer=None):
    ...
    def infer(wav_path):
        ...
        input = get_mel_feature(wav_path).unsqueeze(0)
        input = input.to(device)

        logit = model(input, None, mode='eval')
...
def split_dataset(config, wav_paths, script_paths, valid_ratio=0.005): # 0.05 -> 0.005: 150samples
    train_loader_count = config.workers
    records_num = len(wav_paths) # 30000
    batch_num = math.ceil(records_num / config.batch_size) # 469

    valid_batch_num = math.ceil(batch_num * valid_ratio) # 24
    train_batch_num = batch_num - valid_batch_num # 445
...
def main():
    ...
    parser.add_argument('--lr', type=float, default=1e-7, help='learning rate (default: 0.0001)')
    ...
    train_batch_num, train_dataset_list, valid_dataset = split_dataset(args, wav_paths, script_paths, valid_ratio=0.005) # 0.05 -> 0.005
    ...
    for epoch in range(begin_epoch, args.max_epochs):
        ...
        #nsml.save(args.save_name)
        nsml.save(args.save_name+str(epoch)) # save each epochs
