...
class Model(nn.Module):
    def __init__(self, vocab_len, sos_id, eos_id, d_model=512, nhead=8, num_encoder_layers=6, 
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(1, 64, kernel_size=(1, 3, 3), stride=(1, 1, 1)), #64, 64, 15, 16, 38 
            #nn.Conv3d(1, 64, kernel_size=(), stridr=(), 
            nn.BatchNorm3d(64),
            nn.ReLU(inplace=True),
            nn.Conv3d(64, 64, kernel_size=(1, 3, 3), stride=(1, 1, 1)), #64, 64, 15, 14, 36
            nn.BatchNorm3d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool3d(kernel_size=(1, 1, 2), stride=(1, 1, 2)), #64, 64, 15, 14, 18
            nn.Dropout(0.3),

            nn.Conv3d(64, 128, kernel_size=(1, 3, 3), stride=(1, 1, 1)), #64, 128, 15, 12, 16
            nn.BatchNorm3d(128),
            nn.ReLU(inplace=True),
            nn.Conv3d(128, 128, kernel_size=(1, 3, 3), stride=(1, 1, 1)), #64, 128, 15, 10, 14
            nn.BatchNorm3d(128),
            nn.ReLU(inplace=True),
            nn.Conv3d(128, 128, kernel_size=(1, 3, 3), stride=(1, 1, 1)), #64, 128, 15, 8, 12
            nn.BatchNorm3d(128),
            nn.ReLU(inplace=True),
            nn.MaxPool3d(kernel_size=(1, 1, 2), stride=(1, 1, 2)), #64, 128, 15, 8, 6
            nn.Dropout(0.3),

            nn.Conv3d(128, 256, kernel_size=(1, 3, 3), stride=(1, 1, 1)), #64, 256, 15, 6, 4
            nn.BatchNorm3d(256),
            nn.ReLU(inplace=True),
            nn.Conv3d(256, 256, kernel_size=(1, 3, 3), stride=(1, 1, 1)), #64, 256, 15, 4, 2 
            nn.BatchNorm3d(256),
            nn.ReLU(inplace=True),
            nn.MaxPool3d(kernel_size=(1, 2, 1), stride=(1, 2, 1)), #64, 256, 15, 2, 2
            #nn.Dropout(0.3),
        )
        ...
        self.embedding = nn.Embedding(vocab_len, 256, padding_idx=padding_idx)
        ...
        self.dec_pos_enc = nn.Embedding.from_pretrained(get_sinusoid_encoding_table(max_length+1, 256, 
                                                                    padding_idx=padding_idx), freeze=True)

        self.linear = nn.Linear(256, d_model)
        self.classifier = nn.Linear(d_model, vocab_len)
        self.logsoftmax = nn.LogSoftmax(dim=-1)
        self.RNN = nn.LSTM(512,512,6,bidirectional=True) # num_layer=3
