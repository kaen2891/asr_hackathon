...
def train(model, total_batch_size, queue, criterion, optimizer, device, train_begin, train_loader_count, print_batch=5, teacher_forcing_ratio=1):
    ...
    while True:
        ...
        if rand_for_aug <= 0.5:
            ...
            warped_masked_spectrogram = spec_augment_pytorch.spec_augment(mel_spectrogram=new_feats)
...
def main():
    ...
    model = Model(len(char2index), SOS_token, EOS_token, d_model=d_model, nhead=8, max_seq_len=1024, 
    ...
    nsml.load(checkpoint='model18', session="team185/sr-hack-2019-50000/77")
    ...
    for epoch in range(begin_epoch, args.max_epochs):
        ...
        #nsml.save(args.save_name)
        nsml.save(args.save_name+str(epoch)) 
