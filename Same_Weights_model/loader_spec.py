...
def get_mel_feature(filepath):
    ...
    if feat.size(0) > 1000:
        feat = feat[:1000, :]
    #:1000, :
    #print("mel spec shape is", feat.size())
    return feat
     
    #y, sr = librosa.load(filepath, sr=SAMPLE_RATE)

    #S = librosa.feature.melspectrogram(y=y, n_mels=n_mels, n_fft=win_length, hop_length=hop_length)
    #feat = torch.FloatTensor(S).transpose(0, 1)
    #print("mel spec shape is", feat.size())
'''

def get_spectrogram_feature(filepath, train=False):
...
class BaseDataset(Dataset):
    ...
    def getitem(self, idx):
        ...
        #feat = get_mel_2(self.wav_paths[idx])
        if self.train:
